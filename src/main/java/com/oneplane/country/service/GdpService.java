...
public class GdpService {
    ...
    /** WebClient 주입 생성자 – 작성자: 오수경 */
    public GdpService(WebClient webClient) {
        this.webClient = webClient;
    }

    /**
     * 특정 국가·연도의 GDP 데이터 조회
     * @param isoCode ISO 3자리 국가 코드
     * @param year    조회 연도
     * @return GDP 정보 Map (gdpRaw, gdpFormatted 등)
     * 작성자: 오수경
     */
    public Map<String, Object> getGdpByCountryAndYear(String isoCode, int year) {
        ...
        try {
            ...
            if (dataArray != null && dataArray.isArray() && dataArray.size() > 0) {
                ...
                if (gdpValue != null) {
                    result.put("gdpRaw", gdpValue);
                    result.put("gdpFormatted", NumberFormat.getInstance(Locale.US).format(gdpValue));
                } else {
